//document.querySelector(".navbar_right i").addEventListener("click", check);
...
function showMedCart() {
  ...
  var cartelements = document.getElementsByClassName("medcart");
  for (let i = 0; i < cartelements.length; i++) {
    var element = cartelements[i];
    if (element.style.display === "block") {
      element.style.display = "none";
      document.querySelector(".meddcartinnerdiv").style.display = "none";
    } else {
      displayCartItemsCount(cart);
      updateCartNumber();
      caltotal(cart)
      ...
      if (element.id === "meddcartdiv") {
        setTimeout(() => {
          document.querySelector(".meddcartinnerdiv").style.display = "block";
        }, 10);
      }
    }
  };
